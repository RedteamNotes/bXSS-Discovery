...
- 默认英语界面，可切换中文和法语
- 36 个经审计的发现模板，支持按分类和关键词筛选
- 用编号流程展示范围、模板、查询、参数、分诊和解析顺序
- 在 Google、Bing、DuckDuckGo、Baidu 中单选一个搜索引擎
- 按搜索引擎生成兼容查询，不把同一套 Google dork 原样套到所有引擎
- 详细解释 scope、模板匹配、搜索引擎选择、语法降级和结果过滤的影响
- 队列和历史视图用于保留分诊上下文
- 默认亮色主题，可切换暗色主题
- 单文件静态应用，可直接托管
...
5. 调整查询参数，包括一个选定的搜索引擎和可选结果过滤。生成命令会随搜索引擎变化，避免把不支持的算符静默带过去。
6. 仅在范围和授权状态正确时打开查询。
...
当前模板库包含 36 个经审计的模板，覆盖入口、反馈、支持、信任安全、招聘、市场、销售、身份和运营类发现面。

不同搜索引擎并不共享完全相同的 dork 语法。工具会在适合时保留 Google 的完整查询形式；Bing 使用兼容的 `intitle`、`inbody`、`OR`、`NOT`；DuckDuckGo 使用更聚焦的字段匹配和排除；Baidu 使用保守的 `intitle`、`inurl`、`site` 组合。
